  ...
  const [allFreelancers, setAllFreelancers] = useState([
    {
      ...
      level: 'Expert',
      description:
        'Experienced UI/UX designer with a passion for user-friendly interfaces.',
  ...
  const fetchFreelancers = async () => {
    try {
      const response = await fetch(
        'http://127.0.0.1:8000/freelancers/freelancers/'
      );
      if (!response.ok) {
  ...
  return (
    <div className='bg-[#EAF6FF]'>
      ...
      <div className='flex flex-col items-start px-[10vw] pt-[7vh] pb-[15vh]'>
        <Searchbar
          data={allFreelancers}
          onFilter={setFilteredFreelancers}
          freelancer={true}
        />
        <Categories
          data={allFreelancers}
            ...
            {filteredFreelancers?.map((freelancer) => (
              <div className='bg-white pt-8 pb-8 pl-4 pr-4 shadow-md rounded-2xl overflow-hidden flex flex-col gap-2 h-full w-[60vw]'>
                <div className='flex gap-[1vw]'>
                  <p className='font-bold text-sm text-[#020B56]'>
                    {freelancer.nom}
                  </p>
                  <p className='font-bold text-sm text-[#020B56]'>
                    {freelancer.prenom}
                ...
                <p className='text-[#1AE3D9] text-sm'>
                  {freelancer.profession}
                </p>

                <p className='text-[#1AE3D9] text-sm'>{freelancer.level}</p>

                <p className='text-sm text-[#020B56]'>
                  {freelancer.description}
                </p>

                <div className='flex gap-2 flex-wrap'>
                  {freelancer.Categories.map((cat) => (
                    <p key={cat} className='font-bold text-[#020B56] text-sm'>
                      {cat}
                    </p>
                  ))}
                </div>
                ...
                >
                  <div className='flex gap-[1vw]'>
                    <p className='font-bold text-[#1AE3D9] text-sm'>
                      Voyez mes projets
                    </p>
